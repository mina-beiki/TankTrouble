...
public class Wall {
    ...
    public Wall(int x1, int y1, int x2, int y2 , String type , String lineType) {
        ...
        if(type.equals("brk")) {
            this.lives = Client.getInstance().getUser().getSettings().getBrkWallsLives()  ;
        }else{
            lives = 0 ;
        }
    }
    ...
    /**
     * Decreases the lives number of wall (Used when a bullet hits a breakable wall ) .
     */
