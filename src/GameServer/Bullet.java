...
/**
 * Presents one bullet in the online  with all its features such as location , speed , time
...
public class Bullet {
     ...
     */
    public Bullet(double angle, int xTank, int yTank,MapHandler mapHandler) {
    ...
    public void moveBullet() {
        ...
        int halfBullet = 10; //TODO : this should be set according to the bullet
        x = Math.max(x, 0);
        x = Math.min(x, 360 - halfBullet);
        y = Math.max(y, 0);
        y = Math.min(y, 16 * 360 / 9 - halfBullet);
    ...
    private boolean hitHorizontal(double x, double y) {
        for (Client.gui.Wall wall : GameFrame.getInstance().getHorizontalWalls()) {
