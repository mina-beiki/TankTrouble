 ...
 */
public class Tank {
    private transient Thread thread, moveThread;
     ...
     * location .
     */
    public Tank(String color,MapHandler mapHandler) {
        this.color = color;
        this.mapHandler = mapHandler;
        while (true){
            Random random = new Random();
            tankX=random.nextInt(mapHandler.getGameWidth());
            tankY=random.nextInt(mapHandler.getGameHeight());
            if(initTank()){
                break;
            }
        }
    ...
    public void newBullet(){
        Bullet bullet = new Bullet(angle,tankX,tankY,mapHandler);
        bullet.moveBullet();
        bullets.add(bullet);
    }
    ...
    public void moveTank(String move){
        ...
        setTankX(Math.min(tankX, mapHandler.getGameWidth() - halfTank + 3));
        setTankY(Math.max(tankY, 25));
        setTankY(Math.min(tankY, mapHandler.getGameHeight() - halfTank + 25));
    ...
    private void moveForward(){
        tankYSpeed = Math.sin(Math.toRadians(angle))*speed;
        tankXSpeed = Math.cos(Math.toRadians(angle))*speed;
        if (mapHandler.checkWalls((tankX+tankXSpeed + 10),(tankY+tankYSpeed +10))) {
            tankX += tankXSpeed;
            tankY += tankYSpeed;
        }
    }
    ...
    private boolean initTank (){
        ...
        for(Wall wall : mapHandler.getWalls()){
